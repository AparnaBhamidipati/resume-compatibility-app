...
def check_resume_compatibility(resume: str, job_description: str) -> str:
    """
    ...
    if st.button("Search Jobs"):
        if job_title and location:
            ...
            if isinstance(job_data, list) and job_data:
                # Display job listings with clickable links
                for i, job in enumerate(job_data):
                    if st.button(f"Check Compatibility: {job['Title']} at {job['Company']}"):
        ...
        if resume_file:
            ...
            if st.button("Test Compatibility"):
                with st.spinner("Calculating compatibility..."):
                    compatibility_result = check_resume_compatibility(resume_text, selected_job["Title"])
